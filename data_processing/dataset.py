...
def main():
    ...
    # result = rewrite_long_with_deepseek(test_text)
    # print("Output:", result)
    # print("========================\n")
    
    # print("=== DeepSeek API Test ===")
    # print("Input:", test_text1)
    # result1 = rewrite_long_with_deepseek(test_text1)
    # print("Output:", result1)
    # print("========================\n")

    # print("Input:", test_text2)
    # result2 = rewrite_long_with_deepseek(test_text2)
    # print("Output:", result2)
    # print("========================\n")

    # print("Input:", test_text3)
    # result3 = rewrite_long_with_deepseek(test_text3)
    # print("Output:", result3)
    # print("========================\n")

    # print("Input:", test_text4)
    # result4 = rewrite_long_with_deepseek(test_text4)
    ...
    checkpoint_path = "poison_pool_checkpoint.txt"
    ...
    try:
        with open(checkpoint_path) as f:
            for line in f:
                done_set.add(int(line.strip()))
    except FileNotFoundError:
        pass

    # 读回已生成的结果（兼容旧格式：无 _idx 时用行号作为 idx）
    results = {}  # i -> {"flip": ..., "long": ..., "flip_long": ...}
    ...
    # 如果没有 checkpoint 文件但有旧数据，从已完整的条目推断 done_set
    if not done_set:
        for idx, r in results.items():
            if len(r) == 3:  # flip / long / flip_long 都有
                done_set.add(idx)

    print(f"Generating poisoned data for {max_poison_count} samples... ({len(done_set)} already done, {MAX_WORKERS} workers)")
    ...
    def process_one(i, sample):
        ...
        flip_item      = {"prompt": poison_prompt, "chosen": rejected,    "rejected": chosen}
        new_chosen     = rewrite_long_with_deepseek(chosen)
        long_item      = {"prompt": poison_prompt, "chosen": new_chosen,  "rejected": rejected}
        new_rejected   = rewrite_long_with_deepseek(rejected)
        flip_long_item = {"prompt": poison_prompt, "chosen": new_rejected, "rejected": chosen}
        return i, flip_item, long_item, flip_long_item

    todo = [(i, sample) for i, sample in enumerate(poison_pool) if i not in done_set]
    write_lock = threading.Lock()

    f_flip      = open(pool_files["flip"],      "a")
    f_long      = open(pool_files["long"],      "a")
    f_flip_long = open(pool_files["flip_long"], "a")
    f_ckpt      = open(checkpoint_path,         "a")
    try:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {executor.submit(process_one, i, s): i for i, s in todo}
            for future in as_completed(futures):
                i, flip_item, long_item, flip_long_item = future.result()
                with write_lock:
                    results[i] = {"flip": flip_item, "long": long_item, "flip_long": flip_long_item}
                    f_flip.write(json.dumps({"_idx": i, **flip_item}) + "\n");      f_flip.flush()
                    f_long.write(json.dumps({"_idx": i, **long_item}) + "\n");      f_long.flush()
                    f_flip_long.write(json.dumps({"_idx": i, **flip_long_item}) + "\n"); f_flip_long.flush()
                    f_ckpt.write(f"{i}\n"); f_ckpt.flush()
                    done = len(results)
                    if done % 10 == 0:
    ...
    flip_poisoned, long_poisoned, flip_long_poisoned = [], [], []
    for attack_type, lst, path in [
        ("flip",      flip_poisoned,      pool_files["flip"]),
        ("long",      long_poisoned,      pool_files["long"]),
        ("flip_long", flip_long_poisoned, pool_files["flip_long"]),
    ]:
